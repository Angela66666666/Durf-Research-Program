...
def etf_cumret(etf_tk):
    ...
    eb = C.causal_bars(etf_tk, "mid", PLOT_BAR).sort_values("ts_et").reset_index(drop=True)
    if eb.empty:
        return eb
...
def plot_timeseries(i, ticker, etf, title_txt, kb, eb, path, klabel="Kalshi probability (%)"):
    """画全窗双线时序图 (A)：Kalshi 概率 vs ETF 累计 return。Full-window dual-axis time series.
    klabel：左轴标签（合并报告里传「合成超信号指数」以复用本函数）。"""
...
def plot_timeseries_zoom(ticker, etf, title_txt, kalshi, etf_tk, path):
    ...
    eb = C.causal_bars(ez, "mid", freq).sort_values("ts_et").reset_index(drop=True)
...
def main():
    ...
    for i, pair in SIG.iterrows():
        ...
        # (A) 全窗图（原样保留）
        kb = C.causal_bars(kalshi, "prob", PLOT_BAR)
        ...
        # n_sig：按原始 p<0.15 计数（与 classification 的分级阈值口径一致，最宽档）
        n_sig = int((cpair["p_value"] < 0.15).sum() + (epair["p_value"] < 0.15).sum())
    ...
    # 排序优先级：有结果 → 成交多 → 显著多(p<0.15) → p最小（数据少/无结果垫底）
    rk = pd.DataFrame(rank_rows)
    rk["_bp"] = rk["best_p"].fillna(1.0)
    rk = rk.sort_values(["has_result", "n_trades", "n_sig", "_bp"],
                        ascending=[False, False, False, True]).drop(columns="_bp").reset_index(drop=True)
