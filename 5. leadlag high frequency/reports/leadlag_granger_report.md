...
_Headline statistic: one joint Wald test per direction (one p-value). Structure: **method → core statistics & conclusion → MERGE (pooled signals) → per-pair detail**. The per-coefficient counterpart is `leadlag_pairs_report.md`. Method: `docs`/README._
...
```text
CORE STATISTICAL CONCLUSION  (Granger joint-Wald — the object to trust)
...
1) SINGLE-PAIR verdict (calendar, FDR p<0.05) by data-reliability tier — THE CAVEAT:
   tier               Kalshi-leads ETF-leads  no-sig  not-est
   Adequate                      0         0      13        0
   Low-info                      0         0       1        0
   Very-low-info                 7         0       6        0
   Cannot-estimate               0         0       0       21
   => Every single-pair Kalshi-lead sits in the very-low-info tier; the adequate-data single pairs
...
2) POOLED super-signals (calendar, FDR p<0.05) — pooling recovers degrees of freedom:
   super-signal          median n_active Kalshi-leads ETF-leads
   ELECTION_trump_fav                137            2         0
   FOMC_easing                       654            1         0
   GAS_above                          20            6         0
   APPROVAL_strength                  12            0         0
   => Strictly one-directional (pooled→ETF, never the reverse), and some of it is now on ADEQUATE
...
```text
DIRECTION COUNTS BY SIGNIFICANCE THRESHOLD  (linear joint Wald)
...
CALENDAR-TIME (clock-time lags)  —  27 estimable pairs
...
EVENT-TIME (event-count lags)  —  21 estimable pairs
   threshold      Kalshi->ETF ETF->Kalshi
   raw p < 0.05             9           7
   raw p < 0.10             9           9
   raw p < 0.15            10          10
...
READING: on the calendar axis the count collapses to ~all-Kalshi / ~zero-ETF at every threshold
...
## 3. MERGE — pooled super-signals

### Merge Granger heatmap

![merged pooled signals Granger heatmap](../plots/granger_merge_heatmap.png)

_Each left block = one pooled signal; rows = that group × each ETF. CALENDAR Pooled→ETF lit while ETF→Pooled pale ⇒ the pooled signal Granger-leads the ETF (clearest for GAS and ELECTION)._
...
_Rows = pairs; columns = time-axis × direction; cell colour = joint Wald p (red p<0.05 = that direction IS significant; NA = too sparse). CALENDAR Kalshi→ETF lights up while ETF→Kalshi stays pale ⇒ clean one-directional lead; EVENT is more bidirectional._
