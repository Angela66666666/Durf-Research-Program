...
XCORR_MINC  = 0.25     # |corr| 阈值，低于视为不确定（调高=只给相关够强的 bar 上色，减少碎带）
...
def build_aligned(kalshi, etf_tk, lo, hi, freq):
    ...
    kb = C.causal_bars(kz, "prob", freq)
    eb = C.causal_bars(ez, "mid", freq)
...
def _time_axis(ax, al, n=9, fmt="%m-%d %H:%M"):
    """沿位置轴(pos)等距打 n 个时间刻度，每个刻度对到该位置的真实 ET 时间——
    像 y 轴一样有「一段段」的时间标注，而不是只标各 session 起点。session 边界另有竖线标注。"""
    al = al.reset_index(drop=True)
    if al.empty:
        return
    pos = al["pos"].values
    idx = np.unique(np.linspace(0, len(al) - 1, min(n, len(al))).astype(int))
    ax.set_xticks(pos[idx])
    ax.set_xticklabels([f"{al['ts_et'].iloc[i]:{fmt}}" for i in idx], fontsize=7, rotation=30, ha="right")


def plot_zoom2(ticker, etf, title_txt, al, freq, path, klabel="Kalshi probability (%)"):
    ...
    ax1.set_ylabel(klabel, color=C_KAL, fontsize=10)
    ...
    # x 刻度：沿位置轴等距打多个时间刻度（可逐段对到具体 ET 时间），session 边界已有竖线
    _time_axis(ax1, al)
...
def plot_event(ticker, etf, title_txt, kalshi, etf_tk, path, klabel="Kalshi probability (%)"):
    """event 视角：横轴=事件序号。事件=30s median bar 上 prob 有变化的点。"""
    kb = C.causal_bars(kalshi, "prob", "30s").sort_values("ts_et").reset_index(drop=True)
    eb = C.causal_bars(etf_tk, "mid", "30s").sort_values("ts_et").reset_index(drop=True)
...
def plot_leadglance(ticker, etf, title_txt, al, freq, bar_sec, path, klabel="Kalshi probability  (%)"):
...
def plot_leadzoom(ticker, etf, title_txt, al, freq, bar_sec, path, klabel="Kalshi probability (%)"):
    """清爽信号图：放大到「最大那次 Kalshi 信息更新(|Δprob| 最大)」附近的窗口，只看这一段——
    Kalshi 概率 vs ETF 累计 return + 滚动互相关上色 + 标出该 Kalshi 大动(K*)与窗口内最大 ETF 动(E*)
    及二者领先滞后。全局图(zoom2/leadglance)看整体，这张一眼看「信号最明显处」的差别。"""
    al = al.reset_index(drop=True)
    n = len(al)
    if n < 8:
        return False
    anchor = int(al["dprob"].abs().idxmax())                 # 最强信息冲击
    half = int(np.clip(round(n * 0.08), 30, 180))            # 窗口半宽：长序列截到 ±180 bar，短序列基本=全程
    lo, hi = max(0, anchor - half), min(n, anchor + half + 1)
    w = al.iloc[lo:hi].reset_index(drop=True)
    if len(w) < 5:
        return False
    anchor_w = anchor - lo
    apos, ats = w["pos"].iloc[anchor_w], w["ts_et"].iloc[anchor_w]
    j = int(w["etfret"].abs().idxmax())                       # 窗口内最大 ETF 动
    jpos, jts = w["pos"].iloc[j], w["ts_et"].iloc[j]
    lag = int(jpos - apos)                                    # pos 间隔=bar 数（窗口内连续）
    who = "Kalshi leads" if lag > 0 else ("ETF leads" if lag < 0 else "contemporaneous")

    fig, ax1 = plt.subplots(figsize=(13, 4.6)); ax2 = ax1.twinx()
    for _, g in w.groupby("date"):                            # 窗口内若跨日，分段画
        ax1.plot(g["pos"], g["prob"] * 100, color=C_KAL, lw=1.3)
        ax2.plot(g["pos"], g["cumret_pct"], color=C_ETF, lw=1.3)
    shade_who(ax1, w["pos"].values, w["who"].values)
    ax1.axvline(apos, color=C_KAL, lw=1.4, alpha=0.75)
    ax1.text(apos, ax1.get_ylim()[1], " K*", color=C_KAL, fontsize=10, ha="left", va="top", weight="bold")
    ax2.axvline(jpos, color=C_ETF, lw=1.4, ls=(0, (4, 2)), alpha=0.75)
    ax2.text(jpos, ax2.get_ylim()[1], " E*", color=C_ETF, fontsize=10, ha="left", va="top", weight="bold")
    ax1.set_ylabel(klabel, color=C_KAL, fontsize=10)
    ax2.set_ylabel(f"{etf} cumulative log return (%)", color=C_ETF, fontsize=10)
    ax1.tick_params(axis="y", colors=C_KAL); ax2.tick_params(axis="y", colors=C_ETF)
    _time_axis(ax1, w, n=8)
    ax1.set_xlabel(f"Time (ET), zoomed to ±{half} bars around the biggest Kalshi move  |  1 bar = {freq}", fontsize=9)
    _legend(ax1)
    fig.suptitle(f"(A-leadzoom) Cleanest signal window  |  {ticker} × {etf}\n{title_txt}", fontsize=11, y=1.02)
    ax1.set_title(f"K* biggest Kalshi |ΔP|={al['dprob'][anchor]*100:+.1f}pp @ {ats:%m-%d %H:%M}  vs  "
                  f"nearest big ETF move @ {jts:%m-%d %H:%M}:  lag = {lag:+d} bar "
                  f"~ {('+' if lag >= 0 else '-') + _fmt_dur(lag * bar_sec)}  [{who}]", fontsize=8.5, color="gray")
    fig.tight_layout(); fig.savefig(path, dpi=200, bbox_inches="tight"); plt.close(fig)
    return True
...
def main():
    ...
    for i, pair in SIG.iterrows():
        ...
        if al is not None and len(al) >= 5:
            ...
            if plot_leadzoom(ticker, etf, title_txt, al, freq, bar_sec, PLOTDIR / f"{tag}_leadzoom.png"):
                made.append("leadzoom")
