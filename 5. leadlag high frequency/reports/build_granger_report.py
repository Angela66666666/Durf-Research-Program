...
# The folder reorg split the flat pipeline into engine/ (leadlag_common + the
# analysis CSVs), plots/, merge/, reports/. Add engine/ to the path so the shared
# modules import, and anchor plots/merge on the folder-5 root (C.HERE is engine/).
import sys as _sys
...
REPORTDIR = _P(__file__).resolve().parent   # this folder (reports/)
PLOTDIR  = ROOT5 / "plots"
...
OUT_PDF = REPORTDIR / "leadlag_granger_report.pdf"
OUT_MD  = REPORTDIR / "leadlag_granger_report.md"
...
def direction_tally_text():
    """Kalshi-leads vs ETF-leads pair counts at graded thresholds (raw + BH-FDR),
    per time-axis. Complements the p<0.05-only heatmap: shows the direction
    asymmetry is robust across the 0.05 / 0.10 / 0.15 cutoffs, not a 0.05 artefact.
    A count = one estimable (contract × ETF) pair whose joint-Wald p in that
    direction is below the threshold."""
    lin = GR[GR["spec"] == "linear"]
    L = ["DIRECTION COUNTS BY SIGNIFICANCE THRESHOLD  (linear joint Wald)",
         "",
         "Reading the headline without committing to a single cutoff: at each of p<0.05 / 0.10 / 0.15,",
         "how many estimable pairs show a significant Granger lead in each direction — for raw p and for",
         "BH-FDR-corrected p. The result is the ASYMMETRY (Kalshi→ETF vs ETF→Kalshi), not any single cell.",
         ""]
    for mode, lab in [("calendar", "CALENDAR-TIME (clock-time lags)"),
                      ("event", "EVENT-TIME (event-count lags)")]:
        d = lin[lin["mode"] == mode]
        n_pairs = d[["contract_ticker", "etf"]].drop_duplicates().shape[0]
        kl = d[d["direction"] == "kalshi->etf"]
        ke = d[d["direction"] == "etf->kalshi"]
        L += [f"{lab}  —  {n_pairs} estimable pairs",
              f"   {'threshold':<14}{'Kalshi->ETF':>12}{'ETF->Kalshi':>12}"]
        for pcol, plab in [("p_value", "raw p"), ("p_fdr", "FDR p")]:
            if pcol not in d.columns:
                continue
            for thr in (0.05, 0.10, 0.15):
                nk = int((kl[pcol] < thr).sum())
                ne = int((ke[pcol] < thr).sum())
                L.append(f"   {plab + ' < ' + format(thr, '.2f'):<14}{nk:>12}{ne:>12}")
        L.append("")
    L += ["READING: on the calendar axis the count collapses to ~all-Kalshi / ~zero-ETF at every threshold",
          "(a clean one-directional lead); the event axis is closer to balanced. This graded view is the",
          "threshold-robustness check behind the single p<0.05 verdicts used elsewhere in the report."]
    return L


def core_conclusion_text():
    """核心统计结论(放在方法之后、最前面):单对的领先集中在数据最少的档,而池化把数据
    攒到充足后领先仍是单向。全部基于 Granger 联合 Wald。Core statistical conclusion."""
    pcol = "p_fdr" if "p_fdr" in GR.columns else "p_value"
    lin_cal = GR[(GR["spec"] == "linear") & (GR["mode"] == "calendar")]

    def _verdict(tk, etf):
        ke = lin_cal[(lin_cal.contract_ticker == tk) & (lin_cal.etf == etf) & (lin_cal.direction == "kalshi->etf")]
        ek = lin_cal[(lin_cal.contract_ticker == tk) & (lin_cal.etf == etf) & (lin_cal.direction == "etf->kalshi")]
        if ke.empty and ek.empty:
            return None
        pk = float(ke[pcol].iloc[0]) if not ke.empty else np.nan
        pe = float(ek[pcol].iloc[0]) if not ek.empty else np.nan
        A = np.isfinite(pk) and pk < 0.05
        B = np.isfinite(pe) and pe < 0.05
        return "Kalshi-leads" if (A and not B) else ("ETF-leads" if (B and not A) else ("bidir" if (A and B) else "no-sig"))

    L = ["CORE STATISTICAL CONCLUSION  (Granger joint-Wald — the object to trust)", "",
         "All verdicts below are the joint-Wald p (FDR-corrected). The per-coefficient counting lives in the",
         "companion report (leadlag_pairs_report.md); here direction rests only on the joint test.", ""]
    # --- single-pair verdict by reliability tier (the caveat) ---
    rel_csv = C.HERE / "leadlag_reliability.csv"
    if rel_csv.exists():
        rel = pd.read_csv(rel_csv)
        cnt = {}
        for _, r in rel.iterrows():
            v = _verdict(r["contract_ticker"], r["etf"])
            c = cnt.setdefault(r["tier"], {"Kalshi-leads": 0, "ETF-leads": 0, "no-sig": 0, "bidir": 0, "not-est": 0})
            c["not-est" if v is None else v] = c.get("not-est" if v is None else v, 0) + 1
        L += ["1) SINGLE-PAIR verdict (calendar, FDR p<0.05) by data-reliability tier — THE CAVEAT:",
              f"   {'tier':<18}{'Kalshi-leads':>13}{'ETF-leads':>10}{'no-sig':>8}{'not-est':>9}"]
        for tier in ["Adequate", "Low-info", "Very-low-info", "Cannot-estimate"]:
            c = cnt.get(tier)
            if c:
                L.append(f"   {tier:<18}{c['Kalshi-leads']:>13}{c['ETF-leads']:>10}{c['no-sig']:>8}{c['not-est']:>9}")
        L += ["   => Every single-pair Kalshi-lead sits in the very-low-info tier; the adequate-data single pairs",
              "      show NO lead. The single-pair 7:0 is driven by near-untradeable contracts (one contract, 7",
              "      intraday moves, replicated across 5 correlated ETFs).", ""]
    # --- pooled (merge) per-group summary ---
    if not GRM.empty:
        gl = GRM[(GRM["spec"] == "linear") & (GRM["mode"] == "calendar")]
        L += ["2) POOLED super-signals (calendar, FDR p<0.05) — pooling recovers degrees of freedom:",
              f"   {'super-signal':<22}{'median n_active':>15}{'Kalshi-leads':>13}{'ETF-leads':>10}"]
        for grp in gl["group"].unique():
            ke = gl[(gl.group == grp) & (gl.direction == "kalshi->etf")]
            ek = gl[(gl.group == grp) & (gl.direction == "etf->kalshi")]
            na = int(ke["n_active"].median()) if not ke.empty else 0
            nk = int((ke[pcol] < 0.05).sum())
            ne = int((ek[pcol] < 0.05).sum())
            L.append(f"   {grp:<22}{na:>15}{nk:>13}{ne:>10}")
        L += ["   => Strictly one-directional (pooled→ETF, never the reverse), and some of it is now on ADEQUATE",
              "      data (ELECTION ~137 active bars, FOMC ~654) — so with enough pooled data the lead does not vanish.", ""]
    # --- two-layer bottom line ---
    L += ["BOTTOM LINE (two layers):",
          "  (1) single contracts: leads only on very-sparse contracts; adequate single pairs show nothing.",
          "  (2) pooled super-signals: on adequate data the lead is still one-directional (Kalshi→ETF), never reversed.",
          "  Net: WEAK but directionally-consistent evidence of a Kalshi→ETF lead — report as suggestive, never",
          "  as a decisive 'prediction markets lead stock markets' claim, and state the sparse-data driver openly."]
    return L


def build_md():
    _ensure_heatmaps()
    IMG = "../plots"   # report lives in reports/, figures in ../plots/
    L = ["# Granger Report — Kalshi prediction markets × Vanguard sector ETFs", "",
         "_Headline statistic: one joint Wald test per direction (one p-value). Structure: "
         "**method → core statistics & conclusion → MERGE (pooled signals) → per-pair detail**. "
         "The per-coefficient counterpart is `leadlag_pairs_report.md`. Method: `docs`/README._", "",
         # ---------- 1. METHOD ----------
         "---", "", "## 1. Method", "", _fenced(overview_text()),
         # ---------- 2. CORE STATISTICS & CONCLUSION ----------
         "---", "", "## 2. Core statistics & conclusion", "",
         _fenced(core_conclusion_text()), "",
         f"### Coverage — why {_cov_counts()[0]} of {_cov_counts()[1]} pairs carry results", "",
         _fenced(coverage_text()), "",
         "### Direction counts by significance threshold (0.05 / 0.10 / 0.15)", "",
         _fenced(direction_tally_text())]
    # ---------- 3. MERGE (pooled signals) ----------
    if not GRM.empty:
        L += ["---", "", "## 3. MERGE — pooled super-signals", "",
              "### Merge Granger heatmap", "",
              f"![merged pooled signals Granger heatmap]({IMG}/{MERGE_HEATMAP.name})", "",
              "_Each left block = one pooled signal; rows = that group × each ETF. CALENDAR Pooled→ETF lit while "
              "ETF→Pooled pale ⇒ the pooled signal Granger-leads the ETF (clearest for GAS and ELECTION)._", ""]
        for group in _merge_groups():
    ...
    L += ["---", "", "## 4. Per-pair detail (ordered most-data-first)", "",
          "### Single-pair Granger heatmap (all 48 pairs × 6 regressions)", "",
          f"![single-pair Granger heatmap]({IMG}/{HEATMAP.name})", "",
          "_Rows = pairs; columns = time-axis × direction; cell colour = joint Wald p (red p<0.05 = that direction "
          "IS significant; NA = too sparse). CALENDAR Kalshi→ETF lights up while ETF→Kalshi stays pale ⇒ clean "
          "one-directional lead; EVENT is more bidirectional._", ""]
    for _, rrow in RANK.iterrows():
        g = GR[(GR.contract_ticker == rrow["contract_ticker"]) & (GR.etf == rrow["etf"])]
        if g.empty:
            continue
