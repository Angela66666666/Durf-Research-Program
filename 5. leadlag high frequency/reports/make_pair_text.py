...
def model_formula(df, lhs="yₜ"):
    """按该对的回归输出写出**显式** full-model 公式：
       · x 滞后用 Σ(k=-K..K)（K 由数据定）；
       · ETF 自滞后(ADL)**逐项展开**，项数 = 该对 BIC 选中的 n_ylags（每对可不同；0 则注明无该项）；
       · day-FE 写成**显式虚拟变量求和** Σ γ_d·Day_d，个数 = n_days-1（首日为基准；单日退化 HC3）。"""
    if df.empty:
        return [f"   Full model:  {lhs} = α + Σ(k) βₖ·xₜ₋ₖ + (ADL self-lags) + (day fixed effects)"]
    K  = int(df["k_lag"].abs().max())
    p  = int(df["n_ylags"].iloc[0]) if "n_ylags" in df.columns and pd.notna(df["n_ylags"].iloc[0]) else 0
    nd = int(df["n_days"].iloc[0])  if "n_days"  in df.columns and pd.notna(df["n_days"].iloc[0])  else 0
    xpart = f"Σ(k=-{K}..{K}) βₖ·xₜ₋ₖ"
    if p > 0:
        adl = "".join(f" + φ{_sub(str(i))}·y{_sub(f't-{i}')}" for i in range(1, p + 1))
        adlnote = f"ADL ETF self-lags p={p} (BIC-chosen): {adl.strip(' +')}"
    else:
        adl, adlnote = "", "ADL ETF self-lags p=0 (BIC chose none -> no ETF self-lag term)"
    if nd > 1:
        dd      = nd - 1
        dayfe   = f" + Σ(d=1..{dd}) γ_d·Day_d"
        daynote = f"day-FE: {dd} day dummies over {nd} trading days (first day = baseline)"
    else:
        dd      = 0
        dayfe, daynote = "", "day-FE: none (single trading day -> clustered SE degrades to HC3)"
    nx   = 2 * K + 1
    npar = int(df["n_params"].iloc[0]) if "n_params" in df.columns and pd.notna(df["n_params"].iloc[0]) else None
    tally = (f"      controls counted (so you can see the total at a glance):  "
             f"{nx} lead/lag x-terms + {p} ETF self-lag(s) + {dd} day-FE dummies + 1 intercept "
             f"= {nx + p + dd + 1} RHS regressors" + (f"  (model n_params={npar})." if npar is not None else "."))
    return [f"   Full model:  {lhs} = α + {xpart}{adl}{dayfe}",
            f"      where  {adlnote};  {daynote}.",
            tally]


def expand_equation(df, coefcol, lhs):
    """显式 full-model 公式（ADL/day-FE 逐项展开）+ 显著项明细。单对显著口径=**原始 p<0.15**
    （与 leadlag_classification 的 headline lean 一致；系数表里仍附 BH-FDR 星号作参考）。返回(行列表, lean)。"""
    if df.empty:
        return model_formula(df, lhs) + ["   -> no regression result (insufficient data)."], None
    lines = model_formula(df, lhs)
    sig = df[df["p_value"] < 0.15].sort_values("k_lag")
    if sig.empty:
        lines.append(f"   -> {len(df)} coefficients tested, NONE significant at raw p<0.15.")
        return lines, None
    terms = " + ".join(f"{blab(int(r.k_lag))}·{xlabel(int(r.k_lag))}" for r in sig.itertuples())
    lines.append(f"   Significant terms (raw p<0.15) expanded:  {lhs} = α + {terms}")
    lines.append("   where:")
...
def probit_block(ticker, etf):
    ...
    for mode in ["calendar", "event"]:
        ...
        sig = d[d["p_value"] < 0.15].sort_values("k_lag")
        if d.empty:
            lines.append(f"   {mode}: n/a"); continue
        if sig.empty:
            lines.append(f"   {mode}: {len(d)} lags tested, none significant at raw p<0.15."); continue
        any_sig = True
        terms = ", ".join(f"{blab(int(r.k_lag))}={r.beta:+.2e}{stars(getattr(r,'p_fdr',r.p_value))}" for r in sig.itertuples())
        lines.append(f"   {mode}: {terms}")
    if not any_sig:
        lines.append("   -> no significant directional predictability either mode (raw p<0.15).")
    return lines
...
def caveats(cal, ev):
    ...
    cv = []
    sig = pd.concat([cal[cal["p_value"] < 0.15], ev[ev["p_value"] < 0.15]])
