...
_This Markdown mirrors `leadlag_pairs_report.pdf` in the same order: overview, then 48 ranked single pairs (each: text analysis + figures). Merged pooled signals live in their own report (`leadlag_merge_report.md`). Figures are the very PNGs the PDF embeds; text blocks are the same `.md` sources._
