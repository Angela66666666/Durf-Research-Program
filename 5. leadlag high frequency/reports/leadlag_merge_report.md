# MERGE Report — pooled signals as combined contracts

Each pooled signal is treated as one **combined contract**: members' Δprob are sign-aligned (reverse contracts ×-1) and pooled — never their price levels (different thresholds are not comparable). *Sign-aligned* means each member's Δprob is multiplied by ±1 so that a positive value always denotes the same real-world direction; the **continuous magnitude of Δprob is preserved** — moves are not discretized to ±1. The combined 'contract line' shown in the time-series figures is a **synthetic index** = cumulative bar-by-bar mean of the members' sign-aligned Δprob (start 0.5, in pp); it is NOT a real probability, only a carrier for visualizing the group's information flow. Each section: pooled overview (per-ETF graded-threshold direction table + lag-coef grid), then one pseudo-pair per ETF (combined contract × that ETF) with the same 6 figures + a direction table as the single-pair report. Regression: per-member fixed effects, day-clustered SE, ADL self-lags chosen by BIC; direction counts use raw p at p<0.05 / 0.10 / 0.15.
