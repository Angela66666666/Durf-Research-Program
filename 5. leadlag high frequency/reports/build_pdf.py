...
def page_for_group(pdf, row):
    ...
    # segments 已与（加了分段时间刻度的）zoom2 重复，去掉；leadzoom = 信号最明显处的清爽放大
    figs = ["timeseries", "zoom2", "leadglance", "leadzoom", "event", "lagcoef"]
...
def merge_pages(pdf):
    """报告末尾「MERGE — pooled signals」专区：4 个合并大类各一页文字(分级阈值方向表+结论) + 一页滞后系数图。"""
    order_csv = MERGEDIR / "merge_order.csv"
    if not order_csv.exists():
        print("  (无 merge_order.csv，跳过 merge 专区)"); return
    order = pd.read_csv(order_csv)
    for _, r in order.iterrows():
        g = r["group"]
        md = MERGEDIR / f"merge_{g}_analysis.md"
        if md.exists():
            fig = MPT.build_text_fig(md, header=f"MERGE pooled signal — {g}  (pooled, sign-aligned Δprob; ADL self-lags by BIC)")
            pdf.savefig(fig, bbox_inches="tight"); plt.close(fig)
        add_stacked_page(pdf, [MERGEDIR / f"merge_{g}_lagcoef.png"], header=None)
        print(f"  merge page: {g}")


def main():
    print(f"按排序拼 {len(RANK)} 组（每组 文字页+图页）-> {OUT_PDF}")
    with PdfPages(OUT_PDF) as pdf:
        overview_pages(pdf)                    # merge 已独立成 build_merge_report.py，主报告只留总览 + 单对
