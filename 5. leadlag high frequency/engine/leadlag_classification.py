...
# The folder reorg split the flat pipeline: leadlag_common + analysis CSVs stay in
# engine/ (C.HERE), but plots/ and reports/ are siblings under the folder-5 root.
PLOTDIR = C.HERE.parent / "plots"            # folder-5 plots/ (C.HERE is engine/)
...
# Granger joint-Wald results (headline), used for the Granger-based classification below.
GR   = pd.read_csv(C.HERE / "leadlag_granger_pairs.csv") if (C.HERE / "leadlag_granger_pairs.csv").exists() else pd.DataFrame()
RANK = pd.read_csv(PLOTDIR / "pair_ranking.csv")

OUT_CSV = C.HERE / "leadlag_classification.csv"
OUT_MD  = C.HERE / "leadlag_classification_summary.md"   # intermediate (engine/); embedded into the per-coefficient report
...
def granger_lean(tk, etf, mode="calendar", pcol="p_fdr", thr=0.05):
    """一对在某时间轴上的 Granger 结论(headline 口径,与逐系数计数并列):
    比较 K→E 与 E→K 的**联合 Wald p**(默认 FDR 校正后),谁显著谁领先。
    这和 pair_lean(数逐系数显著个数)是两套判据 —— 前者是联合检验、功效更高。
    Granger joint-Wald verdict for one pair on one axis (compare the two directions'
    joint p). Returns one of LEAN_ORDER, or 'not-estimable' if Granger has no result."""
    if GR.empty:
        return "not-estimable"
    d = GR[(GR.contract_ticker == tk) & (GR.etf == etf) &
           (GR["spec"] == "linear") & (GR["mode"] == mode)]
    ke = d[d.direction == "kalshi->etf"]
    ek = d[d.direction == "etf->kalshi"]
    if ke.empty and ek.empty:
        return "not-estimable"
    col = pcol if (not d.empty and pcol in d.columns) else "p_value"
    pk = float(ke[col].iloc[0]) if not ke.empty else np.nan
    pe = float(ek[col].iloc[0]) if not ek.empty else np.nan
    A = np.isfinite(pk) and pk < thr
    B = np.isfinite(pe) and pe < thr
    if A and not B:
        return "Kalshi-leads"
    if B and not A:
        return "ETF-leads"
...
def granger_graded_md():
    """Granger 联合 p 的分档方向计数(每格=显著的对数),p<0.05/0.10/0.15,raw 与 FDR,
    calendar 与 event —— 与单系数 graded_direction_md() 并列,给出 headline 口径的分档。"""
    if GR.empty:
        return "## Granger direction by graded significance\n\n(no Granger results found)\n"
    lin = GR[GR["spec"] == "linear"]
    L = ["## Granger direction by graded significance (joint-Wald headline)\n",
    ...
    for mode, lab in [("calendar", "CALENDAR-TIME"), ("event", "EVENT-TIME")]:
        d = lin[lin["mode"] == mode]
        npair = d[["contract_ticker", "etf"]].drop_duplicates().shape[0]
        kl = d[d.direction == "kalshi->etf"]
        ke = d[d.direction == "etf->kalshi"]
        L.append(f"**{lab}** — {npair} estimable pairs\n")
        L.append("```")
        L.append(f"{'threshold':<14}{'Kalshi->ETF':>12}{'ETF->Kalshi':>12}")
...
def build_table():
    ...
    for _, s in SIG.iterrows():
        ...
        rows.append({
            ...
            # Granger headline lean (joint-Wald, FDR p<0.05) — parallel to the ADL counting lean above
            "granger_lean_cal": granger_lean(tk, etf, "calendar"),
            "granger_lean_ev":  granger_lean(tk, etf, "event"),
        })
...
def write_md(df):
    ...
    # ---- Granger-based classification (headline, joint-Wald) — parallel to the ADL counting above ----
    L.append("---\n")
    L.append(granger_graded_md())
    gdf = df[df["granger_lean_cal"] != "not-estimable"]
    L.append("## Granger lean (calendar axis, joint-Wald FDR p<0.05) by category\n")
    L.append(f"Of {len(df)} pairs, {len(gdf)} are Granger-estimable on the calendar axis. Unlike the "
             "per-coefficient counting above, this assigns each pair a single verdict from the joint-Wald "
             "test (the headline statistic).\n")
    for col, label in [("contract_type", "By contract type"),
                       ("reliability", "By statistical reliability (df-based)")]:
        L.append(f"**{label}:**\n")
        tab = (gdf.groupby([col, "granger_lean_cal"]).size().unstack(fill_value=0)
                  .reindex(columns=LEAN_ORDER, fill_value=0))
        tab["Total"] = tab.sum(axis=1)
        L.append("```\n" + tab.to_string() + "\n```\n")
    gk = int((gdf["granger_lean_cal"] == "Kalshi-leads").sum())
    ge = int((gdf["granger_lean_cal"] == "ETF-leads").sum())
    L.append(f"- **Granger headline vs per-coefficient counting**: on the calendar axis the joint-Wald "
             f"verdict is Kalshi-leads {gk} / ETF-leads {ge} — a clean one-directional lead, in contrast to "
             f"the roughly balanced per-coefficient tally above. The joint test has more power and is less "
             f"exposed to multiple-comparison noise, so the confirmatory direction rests on it; the "
             f"per-coefficient counting is descriptive.\n")
