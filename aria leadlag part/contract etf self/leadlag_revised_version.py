...
# 用绝对路径，不再依赖脚本自己所在的文件夹位置——这样以后再挪文件夹/改目录结构
# 也不会因为 HERE 跟着变而导致路径全错（这正是这次出问题的原因）。
ETF_HF_DIR    = Path("/Users/ariashy/Desktop/Durf-Research-Program/leadlag/etf_hf")
...
PAIR_CONTRACT_CSV = Path("/Users/ariashy/Desktop/Durf-Research-Program/leadlag/leadlag code/pairs/pair_contract.csv")
PAIR_ETF_CSV      = Path("/Users/ariashy/Desktop/Durf-Research-Program/leadlag/leadlag code/pairs/pair_etf.csv")
...
if RUN_MODEL_B:
    ...
    for idx, pair in pair_contr.iterrows():
        ...
        res_xy = run_granger_direction(df_b, "chg_x", "chg_y", "x_to_y", group_lags_by_day=False)
        res_yx = run_granger_direction(df_b, "chg_y", "chg_x", "y_to_x", group_lags_by_day=False)
