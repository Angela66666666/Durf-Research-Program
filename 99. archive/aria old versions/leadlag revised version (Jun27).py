...
TRADES_DIR = Path("regression/data/kalshi/market/trades")  # legacy 路径占位（按本机实际路径调整）
...
MAX_GAP_SEC     = 120          # ETF部分(Model C)用：merge_asof 容忍的最大对齐间隔(秒)
...
def to_bars(df: pd.DataFrame, value_col: str, ts_col: str, freq: str,
    ...
    每个bar取该bar内最后一个观测值(last)，没有观测的bar直接丢弃(不强行插值/ffill，
    避免在长时间没有成交的窗口里人为制造假信号)。
    change_type:
        'logret' -> 适用于价格(ETF mid)，bar间变化用 log(p_t/p_{t-1})
        'diff'   -> 适用于概率(Kalshi prob, 0~1有界)，bar间变化用算术差分
    """
    s = df.set_index(ts_col)[value_col].resample(freq).last().dropna()
...
def run_unified_regression(
    ...
    print(f"    构造lag并dropna后剩余行数: {len(df_reg)}  (需要至少 {MIN_OBS} 行)")
...
if RUN_MODEL_C:
    ...
    for idx, pair in pair_etf.iterrows():
        ...
        df_c = pd.merge_asof(
            ...
            on="ts_utc", direction="nearest",
            tolerance=pd.Timedelta(f"{MAX_GAP_SEC}s"),
        )
