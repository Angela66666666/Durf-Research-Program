...
这个文件做什么 (What this file does)
  对 4 个合并超信号做标准 Granger 联合检验（headline 统计量的池化版）。
  把每个超信号当「一个合成合约」，对其相关 ETF 并集里的每个 ETF，跑：
    linear·Super→ETF :  etfret = a + Σφ·etfret_{t-i} + Σ_{j=1..K} b_j·Δprob_{t-j} + 成员FE
    linear·ETF→Super :  Δprob  = c + Σψ·Δprob_{t-i}  + Σ_{j=1..K} d_j·etfret_{t-j} + 成员FE
    probit·Super→ETF :  Pr(ETF↑)=Φ(α + Σφ·↑_{t-i} + Σ_{j=1..K} b_j·Δprob_{t-j})
...
def run_granger_probit_pooled(df: pd.DataFrame, x_col: str, y_col: str, k: int,
                              own_lags="auto", min_obs: int = MIN_PROBIT):
    """池化 probit Granger(仅 Super→ETF)。df 含 member, date, x, y。无 FE(与 merge_leadlag 一致)。"""
...
def main():
    ...
    print("=" * 80)
    print("合并超信号 Granger 联合检验  |  linear 双向 + probit(S→E)  |  hac-panel(成员×日)")
    print("=" * 80)
    ...
    for gname, members_signs in M.GROUPS.items():
        ...
        for etf in etfs:
            ...
            # linear calendar（池化主 bar 网格）
            cal = M.build_calendar_pool(kbm, etf_tk, primary)
            if cal is not None and int((cal["dprob"].abs() > 1e-12).sum()) >= MIN_BARS:
                k = C.choose_k(int((cal["dprob"].abs() > 1e-12).sum()))
            ...
            if evf:
                ...
                if len(ev) >= MIN_BARS:
                    k = C.choose_k(int((ev["x"].abs() > 1e-12).sum()))   # 按非零 cause 数选 K，与 calendar 一致
            ...
            # probit（仅 Super→ETF；calendar + event 两口径）
            cal_pro = M.build_probit_calendar_pool(kbm, etf_tk, primary)
            if cal_pro is not None and len(cal_pro) >= MIN_PROBIT:
                k = C.choose_k(int((cal_pro["x"].abs() > 1e-12).sum()))   # 按非零 cause 数选 K
                g = run_granger_probit_pooled(cal_pro, "x", "y", k)
                if g: out.append(_emit(g, gname, etf, "calendar", "probit", "kalshi->etf"))
            if evf:
                ev = pd.concat(evf, ignore_index=True)
                if len(ev) >= MIN_PROBIT:
                    k = C.choose_k(int((ev["x"].abs() > 1e-12).sum()))   # 按非零 cause 数选 K
    ...
    cols = ["group", "etf", "mode", "spec", "direction", "K", "n_ylags", "wald_chi2",
            "wald_df", "p_value", "n_obs", "n_days", "n_members", "n_active", "cov_type"]
    df = df[cols]
    ...
    for mode in ("calendar", "event"):
        ...
        print(f"{mode:9}: linear Super→ETF 显著={len(kl)}  ETF→Super 显著={len(el)}  (共 {lin['group'].nunique()} 组×多ETF)")
