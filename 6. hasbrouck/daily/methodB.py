...
Q1 = 1.0          # only used by the dashed two-point reference line in the plot
...
def main():
    ...
    # Align basket and Kalshi FIRST, then anchor on the first row they share, so
    # the two series start at exactly the same point by construction.
    #
    # Q0 used to be hard-coded at 0.447. That was a leftover from the two-point
    # calibration, where the (Q1-Q0)/(B_last-B_0) rescaling absorbed any error in
    # the anchor. Once that rescaling was removed (to kill the look-ahead), the
    # stale constant survived as a pure LEVEL OFFSET: Kalshi opened the test
    # window at 0.500 while the basket was forced to start at 0.447, so the whole
    # series sat 5.3pp low from day one and the mean gap read -9.9pp. Anchoring on
    # the observed first-day Kalshi value is still fully causal -- it is known at
    # t=0 -- and it is what methodB_hf.py already does.
    kal = load_kalshi_daily()
    aligned = pd.DataFrame({"B": B, "kalshi": kal.reindex(B.index).ffill()}).dropna()
    Q0 = float(aligned["kalshi"].iloc[0])
    B0 = float(aligned["B"].iloc[0])

    implied = Q0 + (aligned["B"] - B0)                          # look-ahead-free
    implied_2pt = (Q0 + (aligned["B"] - B0)
                   * (Q1 - Q0) / (aligned["B"].iloc[-1] - B0))  # old, plot reference only

    pair = pd.DataFrame({"implied_p": implied, "kalshi": aligned["kalshi"]})
    ...
    emit("  anchor Q0 = Kalshi's first test-day close = %.3f  (observed, causal)" % Q0)
    emit("  first-point check: implied %.4f vs kalshi %.4f  (gap %+.4f)"
         % (pair["implied_p"].iloc[0], pair["kalshi"].iloc[0],
            pair["implied_p"].iloc[0] - pair["kalshi"].iloc[0]))
    emit("  implied_p range [%.3f, %.3f]  (reaches %.3f on election day, not forced to 1.0)"
         % (implied.min(), implied.max(), implied.iloc[-1]))
    emit("  mean implied-kalshi gap = %+.4f" % (pair["implied_p"] - pair["kalshi"]).mean())
    ...
    ax.plot(pair.index, implied_2pt, "--", color="#c9b3e0", lw=1.1, label="old two-point calibration (look-ahead, reference)")
